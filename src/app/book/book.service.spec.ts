...
describe('BookService', () => {
  ...
  it('getSet1() should books', () => {
    service.getSet1().subscribe((response) => expect(response.set1).toEqual(data.set1))
  ...
  it('can test getSet1() get array from  6 objects', () => {
    const dataLength = 6;
    service.getSet1().subscribe((response) => expect(response.set1.data.length).toBe(dataLength))
  ...
  it('can test HttpClient.get',() => {
    const message = 'Http failure response for http://51.250.16.8:4500/books: 401 Unauthorized';

    service.getSet1().subscribe(
      (response) => fail('should fail with 401 error'),
      (err: HttpErrorResponse) => {
        expect(err.status).toBe(401, 'status')
        expect(err.message).toBe(message, 'message')
      }
    )
